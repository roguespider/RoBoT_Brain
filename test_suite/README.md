# RoBoT Brain — Unified Test Suite (`test_suite`)
...
Do NOT run `cargo build -p test_suite` / `cargo run --package test_suite`
...
| Command | Purpose |
|---|---|
| `test_suite` | Full suite: live tool tests + coverage gate + code analysis + lint (default). |
| `test_suite --list` | Quick smoke check: list every server-advertised tool + required fields. |
| `test_suite --probe TOOL` | Introspect one tool's live `inputSchema` — discover required/optional params. |
...
guessing (e.g. `test_suite --probe register_agent` reveals it requires
