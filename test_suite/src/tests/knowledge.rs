...
pub async fn run_knowledge_tests(
    ...
    test_add_knowledge(client, stats, "Files should be imported before processing").await?;
    ...
    test_record_knowledge_application(client, stats, "test_kno_1", true).await?;
    test_record_knowledge_application(client, stats, "test_kno_1", false).await?;
...
async fn test_add_knowledge(
    ...
    match client.call_tool("add_knowledge", serde_json::json!({
        ...
        Ok(_) => {
            ...
            println!("  ✓ add_knowledge('{}...') - SUCCESS", truncated);
            stats.passed += 1;
        }
        Err(e) => {
            println!("  ✗ add_knowledge('{}...') - FAILED: {}", &content[..40.min(content.len())], e);
            stats.failed += 1;
        }
    }
    Ok(())
...
async fn test_get_mature_knowledge(
    ...
    match client.call_tool("get_mature_knowledge", args).await {
        Ok(_) => {
            println!("  ✓ get_mature_knowledge({:?}) - SUCCESS", min_applications);
            stats.passed += 1;
        }
        Err(e) => {
            println!("  ✗ get_mature_knowledge({:?}) - FAILED: {}", min_applications, e);
            stats.failed += 1;
        }
    }
    Ok(())
...
async fn test_record_knowledge_application(
    ...
    match client.call_tool("record_knowledge_application", serde_json::json!({
        ...
        Ok(_) => {
            println!("  ✓ record_knowledge_application({}, {}) - SUCCESS", knowledge_id, successful);
            stats.passed += 1;
        }
        Err(e) => {
            println!("  ✗ record_knowledge_application({}, {}) - FAILED: {}", knowledge_id, successful, e);
            stats.failed += 1;
        }
    }
    Ok(())
