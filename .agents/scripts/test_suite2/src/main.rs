...
{}",
    ...
    tests::memory::run_memory_tests(&mut client, &mut stats, None).await?;
    tests::memory_types::run_memory_types_tests(&mut client, &mut stats, None).await?;
    tests::experience::run_experience_tests(&mut client, &mut stats, None).await?;
    tests::knowledge::run_knowledge_tests(&mut client, &mut stats, None).await?;
    tests::workflow::run_workflow_tests(&mut client, &mut stats, None).await?;
    tests::planner::run_planner_tests(&mut client, &mut stats, None).await?;
    tests::hypothesis::run_hypothesis_tests(&mut client, &mut stats, None).await?;
    tests::reflection::run_reflection_tests(&mut client, &mut stats, None).await?;
    tests::search::run_search_tests(&mut client, &mut stats, None).await?;
    tests::ingestor::run_ingestor_tests(&mut client, &mut stats, None, &env).await?;
    tests::agent::run_agent_tests(&mut client, &mut stats, None).await?;
    tests::error_handling::run_error_handling_tests(&mut client, &mut stats, None).await?;
    tests::mcp_workflow::run_mcp_workflow_tests(&mut client, &mut stats, None).await?;

    // Run new comprehensive tests
    tests::rmcp::run_rmcp_tests(&mut client, &mut stats, None).await?;
    tests::acp::run_acp_tests(&mut client, &mut stats, None).await?;
    tests::agent_simulation::run_agent_simulation_tests(&mut client, &mut stats, None).await?;
    ...
    tests::fresh_start::run_fresh_start_tests(&mut stats).await?;

    // P9-001: Flow A — Basic cognition.
    tests::flow_basic_cognition::flow_basic_cognition(&mut stats).await;

    // P9-002: Flow B — Automatic memory retrieval.
    tests::flow_auto_memory_retrieval::flow_auto_memory_retrieval(&mut stats).await;

    // P9-003: Flow C — Automatic experience capture.
    tests::flow_experience_capture::flow_experience_capture(&mut stats).await;

    // P9-004: Flow D — Recovery (job failure → retry → completion).
    tests::flow_recovery::flow_recovery(&mut stats).await;

    // P9-005: Flow E — Restart recovery.
    tests::flow_restart_recovery::flow_restart_recovery(&mut stats).await;

    // P9-006: Flow F — Cross-session memory.
    tests::flow_cross_session_memory::flow_cross_session_memory(&mut stats).await;

    // P2-001E: verify startup does not mutate test data on production DB.
    tests::startup_durability::run_startup_durability_tests(&mut stats).await?;
    ...
    // T2-27: serde round-trip test for data_contracts::Reflection.
    tests::data_contracts_reflection::run_data_contracts_reflection_tests(&mut client, &mut stats)
        .await?;

    // T2-29: serde round-trip test for data_contracts::LearningUpdate.
    tests::data_contracts_learning_update::run_data_contracts_learning_update_tests(
        &mut client,
        &mut stats,
    )
    .await?;

    // T2-30: adapter integration test for data contracts.
    tests::data_contracts_adapters::run_data_contracts_adapters_tests(&mut client, &mut stats)
        .await?;

    // T1-10B-05: migrated knowledge query text/confidence/ranking unit test (MCP-based).
    // T2-13: serde round-trip test for data_contracts::Observation.
    tests::data_contracts_observation::run_data_contracts_observation_tests(
        &mut client,
        &mut stats,
    )
    .await?;

    // T2-15: serde round-trip test for data_contracts::ContextPacket.
    tests::data_contracts_context_packet::run_data_contracts_context_packet_tests(
        &mut client,
        &mut stats,
    )
    .await?;

    // T2-17: serde round-trip test for data_contracts::MemoryRecord.
    tests::data_contracts_memory_record::run_data_contracts_memory_record_tests(
        &mut client,
        &mut stats,
    )
    .await?;

    // T2-19: serde round-trip test for data_contracts::ExperienceRecord.
    tests::data_contracts_experience_record::run_data_contracts_experience_record_tests(
        &mut client,
        &mut stats,
    )
    .await?;

    // T2-21: serde round-trip test for data_contracts::Plan.
    tests::data_contracts_plan::run_data_contracts_plan_tests(&mut client, &mut stats).await?;
    ...
    // T2-23: serde round-trip test for data_contracts::Decision.
    tests::data_contracts_decision::run_data_contracts_decision_tests(&mut client, &mut stats)
        .await?;

    // T2-25: serde round-trip test for data_contracts::ExecutionResult.
    tests::data_contracts_execution_result::run_data_contracts_execution_result_tests(
        &mut client,
        &mut stats,
    )
    .await?;
    ...
    // T1.34 — CoObOpLoop queue lifecycle: enqueue → get → update → verify.
    tests::cooboploop_queue::run_cooboploop_queue_tests(&mut client, &mut stats).await?;

    // T3.13 — evaluation priority: zero cost → 0.0, max values → > 1.0
    tests::cooboploop_t3_eval::run_cooboploop_eval_tests(&mut client, &mut stats).await?;

    // T5.23 — full loop cycle (start → single_cycle → status).
    tests::cooboploop_t5_cycle::run_t5_cycle_tests(&mut client, &mut stats).await?;

    // T13.18 - a completed task cycle does not terminate the retained mission.
    tests::cooboploop_t13_strategic::run_t13_strategic_tests(&mut client, &mut stats).await?;

    // T14.5 - heartbeat resumes observation after WAIT.
    tests::cooboploop_t14_cycle::run_t14_cycle_tests(&mut client, &mut stats).await?;

    // T15.15 - all tracked capability autonomy levels are returned through MCP.
    tests::cooboploop_t15_autonomy::run_t15_autonomy_tests(&mut client, &mut stats).await?;

    // T10.20 - two cycles with different objectives produce different priorities.
    tests::cooboploop_t10_learning::run_t10_learning_tests(&mut client, &mut stats).await?;
    phase.done();
