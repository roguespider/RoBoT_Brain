#!/usr/bin/env python3
"""
OpenHands Agent connected to RoBoT Brain via MCP

This example demonstrates how to configure an OpenHands agent to use
RoBoT Brain as an MCP tool server, giving it access to:
...
from openhands.sdk import LLM, Agent, Conversation, Event, LLMConvertibleEvent, get_logger
...
def create_robot_brain_agent(
    ...
    """
    Create an OpenHands agent configured to use RoBoT Brain MCP server.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="OpenHands Agent with RoBoT Brain MCP")
    parser.add_argument(
        "--message", "-m",
    ...
    parser.add_argument(
        "--robot-brain-path",
        default=None,
        help="Path to robot_brain binary"
    )
