...
.agents/scripts/test_suite2/ (see `make.sh` line 68).
...
python -m pytest .agents/scripts/test_suite2/ -v
...
# Read Rust report:
python -c "import json; d=json.load(open('.agents/scripts/test_suite2/test_suite_report.json')); print(json.dumps(d['summary'], indent=2))"

# Diff two runs:
python -c "import json; d=json.load(open('test_suite_report.json')); print(d.get('coverage', {}).get('untested_tools', []))"
```
