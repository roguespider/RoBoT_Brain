...
use crate::bridge::rmcp::helpers::{enforcement_error_to_content, tool_output_to_content};
...
impl McpServerHandler {
    ...
    async fn get_workflow(
        ...
        self.enforcer
            .record_workflow_retrieved(&self.session_id, input.purpose.clone())
            .await;
    ...
    #[tool(
        ...
        description = "Store a vector embedding for semantic memory search"
    )]
    async fn store_embedding(
        ...
        if let Err(e) = self.check_workflow_enforcement("store_embedding").await {
            tracing::warn!(
                "Workflow enforcement blocked store_embedding: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn list_embeddings(
        ...
        if let Err(e) = self.check_workflow_enforcement("list_embeddings").await {
            tracing::warn!(
                "Workflow enforcement blocked list_embeddings: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    #[tool(
        ...
        description = "Delete an embedding by memory ID"
    )]
    async fn delete_embedding(
        ...
        if let Err(e) = self.check_workflow_enforcement("delete_embedding").await {
            tracing::warn!(
                "Workflow enforcement blocked delete_embedding: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_embedding_stats(&self) -> ContentBlock {
        if let Err(e) = self.check_workflow_enforcement("get_embedding_stats").await {
            tracing::warn!(
                "Workflow enforcement blocked get_embedding_stats: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::memory::execute_get_embedding_stats(&self.context.database).await {
            Ok(result) => {
                self.record_tool_execution("get_embedding_stats", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    async fn record_experience(
        ...
        if let Err(e) = self.check_workflow_enforcement("record_experience").await {
            tracing::warn!(
                "Workflow enforcement blocked record_experience: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_experience_stats(
        ...
        // Check workflow enforcement first
        if let Err(e) = self
            .check_workflow_enforcement("get_experience_stats")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked get_experience_stats: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::experience::execute_get_experience_stats(input, &self.context.database).await {
            Ok(result) => {
                self.record_tool_execution("get_experience_stats", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    async fn list_experiences(
        ...
        if let Err(e) = self.check_workflow_enforcement("list_experiences").await {
            tracing::warn!(
                "Workflow enforcement blocked list_experiences: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_worker_stats(
        ...
        if let Err(e) = self.check_workflow_enforcement("get_worker_stats").await {
            tracing::warn!(
                "Workflow enforcement blocked get_worker_stats: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_worker_count(&self) -> ContentBlock {
        if let Err(e) = self.check_workflow_enforcement("get_worker_count").await {
            tracing::warn!(
                "Workflow enforcement blocked get_worker_count: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn create_reflection(
        ...
        if let Err(e) = self.check_workflow_enforcement("create_reflection").await {
            tracing::warn!(
                "Workflow enforcement blocked create_reflection: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn analyze_patterns(
        ...
        if let Err(e) = self.check_workflow_enforcement("analyze_patterns").await {
            tracing::warn!(
                "Workflow enforcement blocked analyze_patterns: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_recommendations(
        ...
        if let Err(e) = self.check_workflow_enforcement("get_recommendations").await {
            tracing::warn!(
                "Workflow enforcement blocked get_recommendations: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::search::execute_get_recommendations(input, &self.context.database).await {
            Ok(result) => {
                self.record_tool_execution("get_recommendations", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    async fn list_importable(
        ...
        if let Err(e) = self.check_workflow_enforcement("list_importable").await {
            tracing::warn!(
                "Workflow enforcement blocked list_importable: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn transcribe_audio(
        ...
        if let Err(e) = self.check_workflow_enforcement("transcribe_audio").await {
            tracing::warn!(
                "Workflow enforcement blocked transcribe_audio: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn list_ingested_files(
        ...
        if let Err(e) = self.check_workflow_enforcement("list_ingested_files").await {
            tracing::warn!(
                "Workflow enforcement blocked list_ingested_files: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::ingestor::execute_list_ingested_files(input).await {
            Ok(result) => {
                self.record_tool_execution("list_ingested_files", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    async fn delete_ingested_files(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("delete_ingested_files")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked delete_ingested_files: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::ingestor::execute_delete_ingested_files(input).await {
            Ok(result) => {
                self.record_tool_execution("delete_ingested_files", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    async fn extract_knowledge(
        ...
        if let Err(e) = self.check_workflow_enforcement("extract_knowledge").await {
            tracing::warn!(
                "Workflow enforcement blocked extract_knowledge: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn query_knowledge(
        ...
        if let Err(e) = self.check_workflow_enforcement("query_knowledge").await {
            tracing::warn!(
                "Workflow enforcement blocked query_knowledge: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn record_knowledge_application(
        ...
        // Check workflow enforcement first
        if let Err(e) = self
            .check_workflow_enforcement("record_knowledge_application")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked record_knowledge_application: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        let result =
            tools::knowledge::execute_record_knowledge_application(input, &self.context.knowledge)
                .await;
        if result.success {
            self.record_tool_execution("record_knowledge_application", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn get_knowledge_stats(
        ...
        if let Err(e) = self.check_workflow_enforcement("get_knowledge_stats").await {
            tracing::warn!(
                "Workflow enforcement blocked get_knowledge_stats: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("get_knowledge_stats", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn get_mature_knowledge(
        ...
        // Check workflow enforcement first
        if let Err(e) = self
            .check_workflow_enforcement("get_mature_knowledge")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked get_mature_knowledge: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("get_mature_knowledge", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn add_step_dependency(
        ...
        if let Err(e) = self.check_workflow_enforcement("add_step_dependency").await {
            tracing::warn!(
                "Workflow enforcement blocked add_step_dependency: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("add_step_dependency", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn create_workflow(
        ...
        if let Err(e) = self.check_workflow_enforcement("create_workflow").await {
            tracing::warn!(
                "Workflow enforcement blocked create_workflow: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn add_workflow_step(
        ...
        if let Err(e) = self.check_workflow_enforcement("add_workflow_step").await {
            tracing::warn!(
                "Workflow enforcement blocked add_workflow_step: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_workflow_status(
        ...
        if let Err(e) = self.check_workflow_enforcement("get_workflow_status").await {
            tracing::warn!(
                "Workflow enforcement blocked get_workflow_status: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        let result =
            tools::workflow::execute_get_workflow_status(input, &self.context.workflow_engine)
                .await;
        if result.success {
            self.record_tool_execution("get_workflow_status", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn resume_workflow(
        ...
        if let Err(e) = self.check_workflow_enforcement("resume_workflow").await {
            tracing::warn!(
                "Workflow enforcement blocked resume_workflow: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn cancel_workflow(
        ...
        if let Err(e) = self.check_workflow_enforcement("cancel_workflow").await {
            tracing::warn!(
                "Workflow enforcement blocked cancel_workflow: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn delete_workflow(
        ...
        if let Err(e) = self.check_workflow_enforcement("delete_workflow").await {
            tracing::warn!(
                "Workflow enforcement blocked delete_workflow: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn start_exploration(
        ...
        if let Err(e) = self.check_workflow_enforcement("start_exploration").await {
            tracing::warn!(
                "Workflow enforcement blocked start_exploration: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn get_exploration_status(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("get_exploration_status")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked get_exploration_status: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("get_exploration_status", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn complete_exploration(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("complete_exploration")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked complete_exploration: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("complete_exploration", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn abandon_exploration(
        ...
        if let Err(e) = self.check_workflow_enforcement("abandon_exploration").await {
            tracing::warn!(
                "Workflow enforcement blocked abandon_exploration: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("abandon_exploration", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn add_exploration_hypothesis(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("add_exploration_hypothesis")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked add_exploration_hypothesis: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("add_exploration_hypothesis", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn evaluate_exploration_hypothesis(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("evaluate_exploration_hypothesis")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked evaluate_exploration_hypothesis: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        if result.success {
            self.record_tool_execution("evaluate_exploration_hypothesis", None)
                .await;
        }
        tool_output_to_content(result)
    }
    ...
    async fn promote_finding(
        ...
        if let Err(e) = self.check_workflow_enforcement("promote_finding").await {
            tracing::warn!(
                "Workflow enforcement blocked promote_finding: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn pause_exploration(
        ...
        if let Err(e) = self.check_workflow_enforcement("pause_exploration").await {
            tracing::warn!(
                "Workflow enforcement blocked pause_exploration: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    async fn resume_exploration(
        ...
        if let Err(e) = self.check_workflow_enforcement("resume_exploration").await {
            tracing::warn!(
                "Workflow enforcement blocked resume_exploration: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    #[tool(
        name = "register_skill",
        description = "Register a new skill in the skill registry. Skills represent reusable capabilities."
    )]
    ...
    #[tool(
        name = "discover_skill",
        description = "Create a skill discovered from an experience. Per Architecture §15: Skills emerge from experience."
    )]
    ...
    #[tool(
        name = "get_skill",
        description = "Get details of a specific skill including mastery level, usage statistics, and prerequisites."
    )]
    async fn get_skill(
        &self,
    ...
    #[tool(
        name = "list_skills",
        description = "List all registered skills, optionally filtered by category or enabled status."
    )]
    ...
    #[tool(
        name = "update_skill_mastery",
        description = "Update skill mastery based on execution outcome. Records success or failure for the skill."
    )]
    async fn update_skill_mastery(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("update_skill_mastery")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked update_skill_mastery: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::skills::execute_update_skill_mastery(input, &self.context).await {
            Ok(result) => {
                self.record_tool_execution("update_skill_mastery", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    #[tool(
        ...
        description = "Get skill recommendations based on readiness and usage patterns. Per Architecture §15."
    )]
    async fn get_skill_recommendations(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("get_skill_recommendations")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked get_skill_recommendations: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::skills::execute_get_skill_recommendations(input, &self.context).await {
            Ok(result) => {
                self.record_tool_execution("get_skill_recommendations", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    #[tool(
        name = "execute_skill",
        description = "Execute a skill with provided task and parameters. Per Architecture §15: Skill::execute(&context)."
    )]
    ...
    #[tool(
        ...
        description = "Get comprehensive statistics about the skill registry including mastery distribution."
    )]
    async fn get_skill_stats(
        ...
        if let Err(e) = self.check_workflow_enforcement("get_skill_stats").await {
            tracing::warn!(
                "Workflow enforcement blocked get_skill_stats: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    #[tool(
        name = "apply_skill_decay",
        description = "Apply mastery decay to unused skills. Per Architecture §15: Skills include decay."
    )]
    async fn apply_skill_decay(
        ...
        if let Err(e) = self.check_workflow_enforcement("apply_skill_decay").await {
            tracing::warn!(
                "Workflow enforcement blocked apply_skill_decay: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
    ...
    #[tool(
        name = "enable_disable_skill",
        description = "Enable or disable a skill. Disabled skills cannot be executed."
    )]
    async fn enable_disable_skill(
        ...
        if let Err(e) = self
            .check_workflow_enforcement("enable_disable_skill")
            .await
        {
            tracing::warn!(
                "Workflow enforcement blocked enable_disable_skill: {}",
                e.message
            );
            return enforcement_error_to_content(e);
        }
        ...
        match tools::skills::execute_enable_disable_skill(input, &self.context).await {
            Ok(result) => {
                self.record_tool_execution("enable_disable_skill", None)
                    .await;
                tool_output_to_content(result)
            }
            Err(e) => tool_output_to_content(ToolOutput::error(e)),
    ...
    #[tool(
        name = "search_skills",
        description = "Search skills by query, category, or minimum mastery level."
    )]
