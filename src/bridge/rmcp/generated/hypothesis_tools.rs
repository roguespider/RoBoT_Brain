...
async fn record_observation(
    ...
    if let Err(e) = self.check_workflow_enforcement("record_observation").await {
        tracing::warn!(
            "Workflow enforcement blocked record_observation: {}",
            e.message
        );
...
async fn create_hypothesis(
    ...
    if let Err(e) = self.check_workflow_enforcement("create_hypothesis").await {
        tracing::warn!(
            "Workflow enforcement blocked create_hypothesis: {}",
            e.message
        );
        return enforcement_error_to_content(e);
    }
...
async fn list_hypotheses(
    ...
    if let Err(e) = self.check_workflow_enforcement("list_hypotheses").await {
        tracing::warn!(
            "Workflow enforcement blocked list_hypotheses: {}",
            e.message
        );
        return enforcement_error_to_content(e);
    }
...
async fn list_observations(
    ...
    if let Err(e) = self.check_workflow_enforcement("list_observations").await {
        tracing::warn!(
            "Workflow enforcement blocked list_observations: {}",
            e.message
        );
        return enforcement_error_to_content(e);
    }
...
async fn evaluate_hypothesis(
    ...
    if let Err(e) = self.check_workflow_enforcement("evaluate_hypothesis").await {
        tracing::warn!(
            "Workflow enforcement blocked evaluate_hypothesis: {}",
            e.message
        );
        return enforcement_error_to_content(e);
    }
    ...
    match tools::hypothesis::execute_evaluate_hypothesis(input, &self.context.database).await {
        Ok(result) => {
            self.record_tool_execution("evaluate_hypothesis", None)
                .await;
            tool_output_to_content(result)
        }
        Err(e) => tool_output_to_content(ToolOutput::error(e)),
...
#[tool(
    name = "get_evidence",
    description = "Get a specific evidence record by its ID. Returns full evidence details including associated hypothesis."
)]
...
#[tool(
    name = "list_evidence",
    description = "List all evidence records across hypotheses with optional filters."
)]
