...
#   bash .agents/scripts/make.sh test    → auto-cleanup + build + Rust + Python
#   bash .agents/scripts/make.sh clean   → clean artifacts + rebuild
...
Run `bash .agents/scripts/make.sh gate` (Windows with git-bash) or `bash .agents/scripts/make.sh gate` (Unix).
